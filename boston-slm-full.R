...
#mmatrix2a <-  cbind(mmatrix, as.matrix(W %*% mmatrix[,-1]))
#colnames(mmatrix2a)[15:27]<- paste("lag.", colnames(mmatrix2a)[15:27],sep="")
...
fsdem<-as.formula(paste("log(CMEDV) ~ ", paste(names(boston.c2)[1:26], collapse=" + "), "+f(idx,model=\"slm\", args.slm=args.slm, hyper=hyper.slm)", collapse=""))
...
#SLX model
fslx<-as.formula(paste("log(CMEDV) ~ ", paste(names(boston.c2)[1:26], collapse=" + "), "+f(idx,model=\"iid\")", collapse=""))
...
plot(semm1rw2, FALSE, FALSE, TRUE, FALSE, FALSE, FALSE, FALSE, FALSE);dev.off()

#SDEM model
fsdemrw2<-update(fsdem, . ~ . - var5-var18)
fsdemrw2<- update(fsdemrw2, . ~ . +f(var5, model="rw2", hyper=list(theta=list(param=c(1,1)))) )
...
fslxrw2<- update(fslxrw2, . ~ . +f(var5, model="rw2", hyper=list(theta=list(param=c(1,1)))) )
...
par(mfrow=c(1,3))
plot(semm1rw2$summary.random$NOX[,1:2], type="l", lwd=2, main="SEM model",
   xlab="NOX", ylab="f(NOX)")
lines(semm1rw2$summary.random$NOX[,c(1,4)], lty=2)
lines(semm1rw2$summary.random$NOX[,c(1,6)], lty=2)

plot(sdemm1rw2$summary.random$var5[,1:2], type="l", lwd=2, main="SDEM model",
   xlab="NOX", ylab="f(NOX)")
lines(sdemm1rw2$summary.random$var5[,c(1,4)], lty=2)
lines(sdemm1rw2$summary.random$var5[,c(1,6)], lty=2)

plot(slxm1rw2$summary.random$var5[,1:2], type="l", lwd=2, main="SLX model",
   xlab="NOX", ylab="f(NOX)")
