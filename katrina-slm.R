...
fsdem<-as.formula(paste("y1 ~ ", paste(names(Katrina2)[1:16], collapse=" + "), "+f(idx,model=\"slm\", args.slm=args.slm, hyper=hyper.slm)", collapse=""))
...
#SLX model
fslx<-as.formula(paste("y1 ~ ", paste(names(Katrina2)[1:16], collapse=" + "), "+f(idx,model=\"iid\")", collapse=""))
...
#
tabrho<-data.frame(
   SEM=semm1$summary.hyper[1,c(1,2,3,5)],
   SLM=slmm1$summary.hyper[1,c(1,2,3,5)],
   SDM=sdmm1$summary.hyper[1,c(1,2,3,5)],
   SDEM=sdemm1$summary.hyper[1,c(1,2,3,5)],
...
tabrhotrans[2,]<-tabrhotrans[2,]*(rho.max-rho.min)
